...
const ProjectImage: React.FC<ProjectImageProps> = ({ imgSrc, name }) => (
    imgSrc ? (
        <Image
            src={imgSrc}
            width={200}
...
const ProjectHeader: React.FC<ProjectHeaderProps> = ({ name, period }) => (
    <div className="w-48">
      <h4>{name}</h4>
      <div className="flex flex-col">
        <span>{`${period[0]} - ${period[period.length - 1]}`}</span>
      </div>
    </div>
);
...
const ProjectItem: React.FC<ProjectProps> = ({
  ...
  return (
      <div className="flex flex-col md:flex-row gap-2 md:gap-0">
        <div className="flex flex-col gap-2">
          <div className="flex md:flex-col items-center md:items-start mr-4 gap-6">
            <ProjectImage imgSrc={imgSrc} name={name} />
            <div className="flex flex-col gap-2">
              <ProjectHeader name={name} period={period} />
              <Links repoUrl={repoUrl} webUrl={webUrl} />
            </div>
          </div>
        </div>
